...
exports.changePassword = (req, res, next) => {
  ...
  getUserById(id).then(({ rows }) => {
    if (rows.length === 0) throw ({ code: 400, msg: 'no user with that id' });
    ...
    if (!oldPasswordMatches) throw ({ code: 400, msg: 'You entered a wrong password' });
    if (newPasswordMatches) throw ({ code: 400, msg: 'Your new password and old password are the same' });
